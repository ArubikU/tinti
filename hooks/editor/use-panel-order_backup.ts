...
export function usePanelOrder() {
  ...
  const getPanelConfig = useCallback((panelId: PanelType): PanelConfig => {
    return panelConfigs.find(config => config.id === panelId) || 
           DEFAULT_PANEL_ORDER.find(config => config.id === panelId)!
  }, [panelConfigs])
  // Get panels sorted by order
  const getSortedPanels = useCallback(() => {
    return [...panelConfigs].sort((a, b) => a.order - b.order)
  }, [panelConfigs])

  // Check if panel can move up/down
  const canMovePanelUp = useCallback((panelId: PanelType): boolean => {
    const sorted = [...panelConfigs].sort((a, b) => a.order - b.order)
    const index = sorted.findIndex(p => p.id === panelId)
    return index > 0
  }, [panelConfigs])

  const canMovePanelDown = useCallback((panelId: PanelType): boolean => {
    const sorted = [...panelConfigs].sort((a, b) => a.order - b.order)
    const index = sorted.findIndex(p => p.id === panelId)
